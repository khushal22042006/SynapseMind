...
const MindMapView = ({ selectedText, loading, generatedContent, onGenerate, onExport }) => {
  ...
  // Calculate node positions for a hierarchical mind map layout
  const calculateLayout = (nodes, edges) => {
    ...
    // Central node at level 0
    levels[0] = [centralNode];
    
    // Find direct children of central node
    const centralChildren = edges
      .filter(edge => edge.source === centralNode.id)
      .map(edge => nodes.find(n => n.id === edge.target))
      .filter(Boolean);
    
    levels[1] = centralChildren;
    
    // Find grandchildren
    const level2Nodes = [];
    centralChildren.forEach(child => {
      const grandchildren = edges
        .filter(edge => edge.source === child.id)
        .map(edge => nodes.find(n => n.id === edge.target))
        .filter(Boolean);
      level2Nodes.push(...grandchildren);
    });
    
    levels[2] = level2Nodes;
    ...
    positions[centralNode.id] = {
      x: centerX,
      y: centerY,
      width: 160, // Smaller width
      height: 50, // Smaller height
      color: getNodeColor('central')
    };
    
    // Position level 1 nodes (branches) in a circle around center
    const level1Count = centralChildren.length;
    centralChildren.forEach((node, index) => {
      const angle = (2 * Math.PI * index) / level1Count;
      const radius = 200;
      ...
      // Position level 2 nodes (details) around their parent
      ...
      childNodes.forEach((child, childIndex) => {
        const childAngle = angle - 0.5 + (childIndex / (childNodes.length - 1)) * 1.0;
  ...
  const handlePanStart = (e) => {
    ...
    if (svgRef.current) {
      svgRef.current.classList.remove('grab');
  ...
  const handlePanEnd = () => {
    ...
    if (svgRef.current) {
      ...
      svgRef.current.classList.add('grab');
    }
  };
  ...
  // Fit to view
  const fitToView = () => {
    const nodes = generatedContent?.content?.nodes || [];
    if (nodes.length === 0) return;
    
    // Simplified fit logic for circular layout
    setScale(0.8);
    setTranslate({ x: 50, y: 50 });
  };

  // Draw the mind map using SVG
  const drawMindMap = () => {
    if (!generatedContent || generatedContent.type !== "mindmap" || !svgRef.current) return;
    ...
    edges.forEach((edge, index) => {
      ...
      if (sourcePos && targetPos) {
        ...
        line.setAttribute('x1', sourcePos.x + sourcePos.width/2);
        line.setAttribute('y1', sourcePos.y + sourcePos.height/2);
        line.setAttribute('x2', targetPos.x + targetPos.width/2);
        line.setAttribute('y2', targetPos.y + targetPos.height/2);
        line.setAttribute('stroke', '#6366F1');
        line.setAttribute('stroke-width', '2');
        line.setAttribute('opacity', '0.6');
        line.classList.add('mindmap-edge');
    ...
    nodes.forEach(node => {
      ...
      group.classList.add(`mindmap-node-${node.type}`); // Add type-specific class
      group.dataset.nodeId = node.id;
      
      // Add tooltip with full label
      const title = document.createElementNS('http://www.w3.org/2000/svg', 'title');
      title.textContent = node.full_label || node.label;
      ...
      rect.classList.add('mindmap-node-bg');
      ...
      // Create text for node label with word wrapping
      ...
      text.setAttribute('font-size', node.type === 'central' || node.type === 'main' ? '12' : '11');
      text.setAttribute('font-weight', node.type === 'central' || node.type === 'main' ? '600' : '400');
      text.setAttribute('style', 'user-select: none; pointer-events: none;');
      text.classList.add('mindmap-node-text');
      text.classList.add('mindmap-text-wrapper');
      ...
      if (line.trim()) {
        const tspan = document.createElementNS('http://www.w3.org/2000/svg', 'tspan');
        tspan.setAttribute('x', pos.width / 2);
        tspan.setAttribute('dy', lineCount > 0 ? '1.2em' : '0');
        tspan.textContent = line.trim();
        text.appendChild(tspan);
      }
      
      // Adjust vertical position based on line count
      if (lineCount > 0) {
        text.setAttribute('y', (pos.height - (lineCount * 16)) / 2);
      }
  ...
  // Redraw mind map when content changes or expanded
  reactExports.useEffect(() => {
    if (generatedContent && generatedContent.type === "mindmap") {
      setTimeout(drawMindMap, 100); // Small delay to ensure DOM is ready
    }
  }, [generatedContent, isExpanded, scale, translate]);
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mt-4 animate-fadeIn", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between mb-3", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "font-semibold text-slate-200", children: "Interactive Mind Map" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-2", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        ...
        {
          ref: containerRef,
          className: `mindmap-container bg-slate-800 border border-slate-700 rounded-lg ${isExpanded ? "h-96" : "h-64"} overflow-hidden relative`,
          children: /* @__PURE__ */ jsxRuntimeExports.jsxs(reactExports.Fragment, { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "svg",
              {
                ...
                width: "100%",
                height: "100%",
                className: "mindmap-svg grab", // Add grab class
                style: { cursor: 'grab' }
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { 
              className: "mindmap-zoom-controls", // Use CSS class from stylesheet
              children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "button",
                  {
                    onClick: () => handleZoom(0.2),
                    className: "mindmap-zoom-btn", // Use CSS class from stylesheet
                    title: "Zoom In",
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mt-3 pt-3 border-t border-slate-700 text-xs text-slate-400 flex justify-between", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) })
      ] })
    ] });
  };
...
const HistoryView = () => {
  ...
  const [expandedMindMaps, setExpandedMindMaps] = reactExports.useState({});
    ...
    try {
      chrome.storage.local.get(["summaries", "mindmaps"], (result) => {
        console.log("📊 Loaded history:", {
          ...
          mindmaps: result.mindmaps?.length || 0
        });
        
        // Debug: Show what's actually stored
        if (result.summaries && result.summaries.length > 0) {
          console.log("📝 First summary item:", {
            hasText: !!result.summaries[0].text,
            textLength: result.summaries[0].text?.length,
            hasSummary: !!result.summaries[0].summary,
            summaryLength: result.summaries[0].summary?.length,
            preview: result.summaries[0].summary?.substring(0, 100)
          });
        }
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
      ...
      history.summaries.length > 0 ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-2", children: [
        history.summaries.slice(0, 3).map((item, index) => {
          ...
          const summaryText = hasSummary ? item.summary : item.text || "No summary available";
          ...
          return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-slate-800 border border-slate-700 rounded-lg p-3", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex justify-between items-start mb-2", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `px-2 py-0.5 rounded text-xs ${item.level === "quick" ? "bg-green-500/20 text-green-300" : item.level === "detailed" ? "bg-blue-500/20 text-blue-300" : "bg-purple-500/20 text-purple-300"}`, children: item.level || "summary" }),
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mb-2", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-slate-500 mb-1", children: "Original text:" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-slate-400 line-clamp-1", children: 
                item.text && item.text.length > 50 ? item.text.substring(0, 50) + "..." : item.text || "No original text"
              })
            ] }),
            
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mb-3", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-slate-500 mb-1", children: "Summary:" }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-sm text-slate-300", children: [
                isExpanded ? summaryText : (shouldTruncate ? summaryText.substring(0, 200) + "..." : summaryText),
                shouldTruncate && /* @__PURE__ */ jsxRuntimeExports.jsx("button", {
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-xs text-slate-500 flex justify-between items-center", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
        ...
        history.summaries.length > 3 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-center", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { 
          className: "text-sm text-blue-400 hover:text-blue-300",
          onClick: loadHistory, // Refresh to show more
          children: [
            "Show ",
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
      ...
      history.mindmaps.length > 0 ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-2", children: [
        history.mindmaps.slice(0, 2).map((item, index) => {
          const isExpanded = expandedMindMaps[index];
          ...
          return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-slate-800 border border-slate-700 rounded-lg p-3", children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mb-3", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-slate-500 mb-1", children: "Original text:" }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-sm text-slate-300", children: [
                isExpanded || !shouldTruncate ? (item.text || "No text") : (item.text.substring(0, 100) + "..."),
                shouldTruncate && /* @__PURE__ */ jsxRuntimeExports.jsx("button", {
                  onClick: () => toggleMindMap(index),
                  className: "ml-2 text-blue-400 hover:text-blue-300 text-xs",
                  children: isExpanded ? "Show less" : "Show more"
                })
              ] })
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex justify-between items-center", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-xs text-slate-500", children: [
                item.nodes && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "inline-flex items-center gap-1 mr-3", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-2 h-2 rounded-full bg-blue-500" }),
                  item.nodes.filter((n2) => n2.type === "main").length || 0,
                  " main"
                ] }),
                item.nodes && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "inline-flex items-center gap-1", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-2 h-2 rounded-full bg-purple-500" }),
                  item.nodes.filter((n2) => n2.type === "sub").length || 0,
                  " sub"
                ] }),
                item.text && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "ml-3", children: [
                  item.text.length,
                  " chars"
                ] })
              ] }),
        ...
        history.mindmaps.length > 2 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-center", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { 
          className: "text-sm text-blue-400 hover:text-blue-300",
          onClick: loadHistory,
          children: [
            "Show ",
...
const saveMindMap = async (mindMapData) => {
  return new Promise((resolve) => {
    chrome.storage.local.get(["mindmaps"], (result) => {
      ...
      mindmaps.unshift({
        text: mindMapData.text,
        nodes: mindMapData.nodes,
        edges: mindMapData.edges,
        total_concepts: mindMapData.total_concepts,
        timestamp: Date.now()
      });
      
      
      
      console.log("💾 Saving mind map to history:", {
        nodesCount: mindMapData.nodes?.length || 0,
        edgesCount: mindMapData.edges?.length || 0
      });
...
const handleGenerate = async (type, level) => {
  ...
  try {
    if (type === "summary") {
      ...
      console.log("🗺️ Calling generateMindMap API...");
      const result = await generateMindMap(selectedText);
      
      console.log("📋 Mind Map API Result:");
      console.log("- Success:", result.success);
      console.log("- Is mock:", result.isMock);
      ...
      if (result.success) {
        ...
        console.log("🎯 Setting generatedContent with mind map data");
        setGeneratedContent(newContent);
        ...
        await saveMindMap({
          ...
          nodes: result.nodes || [],
          edges: result.edges || [],
          total_concepts: result.total_concepts || 0
        });
